...
public class TalonFXController {
    ...
    public TalonFXController(int motorID) {
        ...
        this.motorVelocityRequest = new VelocityVoltage(0);
        this.motorConfigurator.apply(slot0Configs, 0.05);

        // this.motorConfigurator.apply(null)
        this.motorVelocityRequest.Slot = 0;

        this.motionMagicRequest = new MotionMagicVoltage(0);
        this.motionMagicRequest.Slot = 0;

        this.motorPositionRequest = new PositionVoltage(0);
    ...
    public void configureMotor(double rotorToSensorRatio, double sensorToMechanismRatio,
            double dutyCycleNeutralDeadband, boolean invertClockwise, boolean idleBrake) {
        ...
        motorConfigs.CurrentLimits.SupplyCurrentLowerTime = 0.25;
        motorConfigs.Audio.AllowMusicDurDisable = true;

        StatusCode motorStatus = motorConfigurator.apply(motorConfigs);
        if (!motorStatus.isOK()) {
            DriverStation.reportError("Could not apply  configs, error code:" + motorConfigs.toString(),
                    new Error().getStackTrace());
        }
    }

    public void configurePIDF(double kP, double kI, double kD, double kV, double kS, double kA, double kG,
        ...
        if (!configStatusCode.isOK()) {
            DriverStation.reportError("Could not apply motor configs, error code:" + configStatusCode.toString(),
                    new Error().getStackTrace());
        }
