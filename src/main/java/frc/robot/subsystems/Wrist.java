...
public class Wrist extends SubsystemBase {
  ...
  // Troubleshooting");
  // private GenericEntry sbManualSpeedPercent = wristTab.add("Speed Percent In",
  // 0).withPosition(0, 0).getEntry();
  // private GenericEntry sbManualTargetPosition = wristTab.add("Target Position
  // In", 0).withPosition(1, 0).getEntry();
  // private GenericEntry sbkPIn = wristTab.add("kP In ", 0).withPosition(0,
  // 1).getEntry();
  // private GenericEntry sbkIIn = wristTab.add("kI In ", 0).withPosition(1,
  // 1).getEntry();
  // private GenericEntry sbkDIn = wristTab.add("kD In ", 0).withPosition(2,
  // 1).getEntry();
  // private GenericEntry sbkIZoneIn = wristTab.add("kIz In", 0).withPosition(3,
  // 1).getEntry();
  // private GenericEntry sbkFIn = wristTab.add("kF In", 0).withPosition(4,
  // 1).getEntry();
  // private GenericEntry sbkMinOut = wristTab.add("Min out", 0).withPosition(5,
  // 1).getEntry();
  // private GenericEntry sbkMaxOut = wristTab.add("Max out", 0).withPosition(6,
  // 1).getEntry();

  // private GenericEntry sbkMaxVelocity = wristTab.add("Max Velocity ",
  // 0).withPosition(0, 2).getEntry();
  // private GenericEntry sbkMinVelocity = wristTab.add("Min Velocity",
  // 0).withPosition(1, 2).getEntry();
  // private GenericEntry sbkMaxAccel = wristTab.add("Max Accel",
  // 0).withPosition(2, 2).getEntry();
  // private GenericEntry sbkAllowedError = wristTab.add("Allowed Error",
  // 0).withPosition(3, 2).getEntry();
  ...
  public Wrist() {
    ...
    wristMotor.configureMotor(
        WristConstants.kRotorToSensorRatio,
        WristConstants.kSensorToMechanismRatio,
        WristConstants.kDutyCycleNeutralDeadband,
        WristConstants.kInvertClockwise,
        WristConstants.kIdleBrake);
    wristMotor.configurePIDF(WristConstants.kP, WristConstants.kI, WristConstants.kD, WristConstants.kV,
        WristConstants.kS, WristConstants.kA, WristConstants.kG, WristConstants.kCruiseVel, WristConstants.kAccel);
  ...
  public final void setTargetPosition(final double pos) {
  ...
  public final boolean hasReachedPosition(final double position) {
    return Math.abs(getAbsolutePosition() - position) < WristConstants.kAllowedError;
  }
  ...
  public final void setEnabledSmartMotion(final boolean enabled) {
  ...
  public final Command setEnabledCommand(final boolean enabled) {
    return Commands.runOnce(() -> setEnabled(enabled));
  }

  public final double getAppliedVoltage() {
    return wristMotor.getMotorVoltage();
  }

  public final double getCurrentError() {
    return getTargetPosition() - getPosition();
  }

  public final void incrementPosition(final double increment) {
    setTargetPosition(getTargetPosition() + increment);
  }

  public final Command incrementPositionCommand(final double increment) {
  ...
  // ****************************** MANUAL METHODS
  // ******************************//
  public final void setPercentage(final double per) {
    wristMotor.setSpeed(per);
  }

  public final void setPercentageShuffleBoard() {
    double speed = sbManualSpeedPercent.getDouble(0);
    setPercentage(speed);
    setEnabled(false);
  }

  public final void setPositionShuffleBoard() {
    double targetPosition = sbManualTargetPosition.getDouble(0);
    setTargetPosition(targetPosition);
    setEnabled(true);
  }

  public final void updatePIDFValueShuffleBoard() {
  ...
  public final void initShuffleboard(final int level) {
